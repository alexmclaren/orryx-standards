...
11. Mirror durable memory to the Obsidian vault (see Output Locations — the
    vault-sync step runs after the durable writes, on any run that produced
    output; skip it on a SKIP/ABORT run since nothing changed)
...
- **Obsidian vault mirror** — after the durable writes above land, run
  `pwsh -NoProfile -File D:\state\vault-memory-sync.ps1` (idempotent robocopy
  /MIR; mirrors operator memory → `D:\Vault\main\10-Operator-Memory` and
  copies `DECISIONS.md`/`RISKS.md` → `D:\Vault\main\20-Fleet`). This is the
  auto-trigger that keeps the vault from drifting between runs; the daily
  `OrryxMemoryBackup` scheduled task (20:00, sync + offsite mirror) remains the
  safety net. On a SKIP/ABORT run nothing changed, so skip the sync.
