...
**Routine-specific gate (REPLACES canonical §1's same-day stat, 2026-07-17):** this routine runs Saturday but its sibling producers are weekly on other days (product-review Mon, commercial-review Wed) and deep-research is disabled until ~2026-07-31 — a literal same-day pre-check therefore deadlocks CI permanently (it caused the 2026-07-11 SKIP). The pre-check passes if each required input has a dated file ≤ 7 days old (the Input Freshness Gate then governs severity: FRESH/DEGRADE as normal). SKIP only if a required input's newest dated file is > 7 days old AND its producer is enabled — a disabled producer (e.g. deep-research during the token diet) is noted in §Caveats and excluded, never a skip reason.
