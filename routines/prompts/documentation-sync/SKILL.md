...
## Remote ref hygiene / full-fetch preamble (run BEFORE any git reasoning)

> Canonical: `scheduled-tasks/_shared/REMOTE_REF_HYGIENE.md` — the shared contract
> for how every routine reasons about remote refs. The rules below are this
> routine's copy; if the policy changes, change the canonical and re-propagate.

Once per run, per repo you will touch, before reading any ref:

```
git fetch origin          # NO refspec — all refs
```

**A refspec'd fetch (`git fetch origin main`) silently leaves every OTHER
remote-tracking ref stale, and nothing in later output signals it.**
`git branch -a --contains`, `git log origin/<branch>`, `git rev-parse
origin/<branch>:<path>` and `git branch -r --merged` all answer truthfully
about `refs/remotes/...` as last fetched — which may be hours or days behind
the remote. The command succeeds, the output looks like evidence, and the
finding is wrong.

Rules:
1. **Never assert anything about a remote ref** — exists / contains / merged /
   ahead / behind / blob-at-path — unless a no-refspec `git fetch origin` ran
   **in the same pass**. Not a previous pass, not a previous run.
2. Any finding whose severity depends on remote state must name the fetch that
   backs it. If you cannot, downgrade it to "unverified — no full fetch this
   pass" rather than publishing a severity.
3. Before claiming branch A would clobber file F on merge into B, **prove it**:
   `git merge-tree --write-tree B A`, then read F out of the resulting tree.
   Do not reason from "A contains commit C".
4. Offline / fetch fails: say so, and mark every remote claim "as of local refs,
   last fetched {date}". Do not silently fall back to stale refs.

**Worked example — the failure this exists to prevent (2026-07-31, DOC-38c).**
A run fetched `origin main` only, then used `git branch -a --contains 707bc0c`
to conclude a placeholder template sat on `origin/lane-e/optout-learning-loop`
and would overwrite a good file on merge. It was published 🟠 high, carried a
handoff row, and reached an operator instruction to fix it. A full
`git fetch origin` then showed that branch had **no such file at all**, and
`git merge-tree` showed the merge preserved the good blob. The finding was
void from the moment it was written. Same run, same root cause, earlier:
a stale `origin/main` led the routine to author a fix for a file another
writer had already fixed better — duplicated work, discarded.

Cost of the rule: one command per repo. Cost of skipping it, twice in one run:
one wasted fix and one false 🟠.
...
for the truth anchors below) after a **`git fetch origin`** — no refspec,
all refs, per the Full-fetch preamble above. (This line previously said
`git fetch origin main`; that refspec'd form is what produced the void
DOC-38c finding on 2026-07-31.) If the fetch fails (offline), fall back
to local refs and mark claims "as of local origin/main".

This repo is also a **multi-worktree** repository (`git worktree list` —
`D:\Pillarworks-Enterprise-Website` is the main worktree holding `.git`;
others are linked worktrees sharing one ref store). That shared ref store
is *why* branches appear to switch under you here, and it means a stale
remote-tracking ref misleads every worktree at once.
