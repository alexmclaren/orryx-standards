...
7. **Aged-entry re-verification (added 2026-07-03 — kills ledger rot).** Each run,
   ...
   existence) before re-emitting it. **Run `git fetch origin` (no refspec) in that
   repo first** — `origin/main` is a local cached ref and re-verifying against a
   stale copy is not re-verification; see `_shared/REMOTE_REF_HYGIENE.md`. This
   matters most in exactly the direction this rule cares about: a stale
   `origin/main` hides the very commit that would close the entry, so the
   escalation survives another month on evidence that was already obsolete.
   If reality shows it resolved, close it with
