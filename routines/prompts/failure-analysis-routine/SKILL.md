...
> **HARD vs SOFT — the single source is `routine-schedule.json`, NOT this list
> (aligned 2026-07-16).** This routine's `required_inputs` = **`["eod-summary(N-1)"]`
> ONLY**. That prior-day eod-summary is the *sole* input whose absence gates a SKIP
> (via §1 producer pre-check). Every OTHER file below is **SOFT/preferred**: if it is
> absent (or its producer SKIPped NO_CHANGE, e.g. `engineering-routine` on an empty
> AI-executable queue → no `engineering-{date}.md`), consume the most-recent dated
> copy under the freshness gate and note age — do NOT SKIP the whole run. Do the
> failure analysis on whatever FRESH inputs the day has. *(2026-07-16: this run
> SKIPped citing absent `engineering-{date}.md`, which is not a required_input — a
> soft input was mis-treated as a hard gate. The true hard input, eod-summary(N-1),
> was independently absent, so a SKIP was still correct — but for the right reason.)*
- `D:\reports\daily\engineering-{date}.md`  *(SOFT)*
