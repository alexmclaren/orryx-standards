...
rotation). Three hard gates (these strengthen — they do not replace — the
...
2. **Before accepting a rotation claim as resolved/closed**: cross-check the
   ...
   **Repo evidence cannot close one either.** "Not re-introduced in any delta",
   "HEAD-clean", or a swept-delta pass asserts only that the *source tree* is clean;
   it says nothing about whether the credential was rotated at the provider. A finding
   whose required action includes a rotation stays open until gate #2 passes at the
   provider, even when its code half is fully fixed. *(Observed defect: NEW-13
   Temporal and ESC-011/F-05 Cloudflare were emitted `resolved` on repo evidence for
   4 consecutive reports — 2026-07-31 through 2026-08-04 — dropping the
   "cloud rotation still owed" qualifier the 2026-07-17 report carried explicitly.)*
3. **Never batch a rotation-bearing ID into a compound row.** Compressing several IDs
   into one `resolved` carry-forward row (`NEW-99 / NEW-13 / F-05 / ESC-011 / … —
   resolved`) hides which gate was actually run for which finding, and is how the
   defect above evaded gates #1 and #2. Any finding whose required action includes a
   rotation gets **its own row** with its own `verified_method`; batch rows are for
   non-credential findings only.
